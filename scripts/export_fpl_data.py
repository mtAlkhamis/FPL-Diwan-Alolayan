...
def build_hypothetical_history_df(entries, past_seasons_by_entry, earliest_season=None):
    """Reconstruct a "what if today's league had always existed" past-seasons
    ...
    A manager's personal FPL history can reach back further than THIS
    league's own existence (e.g. they played FPL elsewhere for years before
    joining) - without a cap, those pre-league seasons would still show up
    in the archive, which isn't what "this league's past seasons" should
    mean. earliest_season caps that, the same way the manually-maintained
    CSV path naturally is (it simply never has rows before the league
    existed): pass the league's own first season, e.g. "2013/14" or
    "2025/26", and any season string that sorts before it is dropped.
    "YYYY/YY" strings compare correctly as plain strings. None (default)
    means no cap - every season FPL reports for each manager is included.
    ...
    for entry_id, past_seasons in past_seasons_by_entry.items():
        ...
        for season in past_seasons:
            if earliest_season and season["season_name"] < earliest_season:
                continue
            rows.append(
                {
...
def main(
    ...
    season_history_earliest_season=None,
):
    ...
    # load_season_history. season_history_earliest_season caps how far back
    # the live reconstruction goes (e.g. "2013/14"), so a manager's personal
    # FPL history from before THIS league existed doesn't get pulled in -
    # pass the league's own first season here. Ignored on the CSV path,
    # which is manually curated and never has pre-league rows to begin with.
    if season_history_path and Path(season_history_path).exists():
        ...
        season_history_df = build_hypothetical_history_df(
            entries, past_seasons_by_entry, earliest_season=season_history_earliest_season
        )
